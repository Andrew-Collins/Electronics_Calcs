...
# Title: resistor_vals.py 
#
# Author: Andrew Collins
# 
# Description: Houses dictionary of resistor values for common families, and resistor related functions.
#
...
E24 = [1.0, 1.1, 1.2, 1.3, 1.5, 1.8, 2.0, 2.2, 2.4, 2.7, 3.0, 3.3, 3.6, 3.9, 4.3, 4.7, 5.1, 5.6, 6.2, 6.8, 7.5, 8.2, 9.1]
...
family_accuracy = {"e12": 0.025, "E12": 0.025, "e24": 0.0125, "E24": 0.0125, "e48": 0.005, "E48": 0.005, "e96": 0.0025,"E96": 0.0025}


# Function name: resistor_ratio
#
# Description: Calculates a combination of resistors (either singular, series or parallel), that
#               matches a given ratio as closely as possible.
#
# Inputs: family (string) : label of the resistor family being used. 
#         ratio (float/decimal) : value of the ratio needed. 
#          first_val (float/decimal) : one of the resistor values to be used in the raito. 
#
...
def resistor_ratio(family, ratio, first_val) :
    ...
    if ((family in family_list) and ratio > 0.0) :
        ...
        accuracy = family_accuracy[family]
        ...
        if (int(log10(first_val)) < (-1*int(log10(ratio))) and ratio < 1.0) :
            pow_add = (-1*int(log10(ratio))  - int(log10(first_val)))

        # Calculate highest power of 10 (+1) that the values can get to
        pow_high = int(round((log10(ratio * first_val)), 1)) + 2 + pow_add
        print(pow_high)
        ...
        for part in values:
            ...
            for x in range(0, pow_high) :
                ...
                if (abs(first_val - ratio* y)/first_val < accuracy) :
                ...
                for z in range(0, pow_high) :
                    for item in values:
                        ...
                        if (abs(first_val - ratio * (y + second))/first_val < accuracy):
                            out_series.append([first_val, [y, second]])
                            result = 1
                        elif (abs(first_val - ratio * (second * y)/(second + y))/first_val < accuracy):
